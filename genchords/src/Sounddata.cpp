...
/***
 *  original contructor for Sounddata, genchord using
 *
 * */
...
/***
 *
 * Qt using with QString and convert
 *
 * */
Sounddata::Sounddata(const QString soundfilename) {

    std::cout << std::endl;
    //const QString extension = soundfilename.substr(soundfilename.rfind("."));
    const QStringList part = soundfilename.split(".");
    const QString extension = part[1];
    const QString sndfilename = soundfilename;
...
/***
 * Oroginal init for orignial call
 *
 * */
 ...
 *
 * */
void Sounddata::init(const QString soundfilename) {
	delete [] mSrcBuffer;
...
/***
 * import mp3 with QString
 * */
void Sounddata::importMP3(const QString mp3name) {
	// __TODO__: Create only temporary wav file and delete it afterwards
    // __TODO__: use libmad instead of lame. don't create wav file
	 
	std::cout << "-------Decoding MP3 file using L.A.M.E.-----------" <<std::endl;
    QString wavname = mp3name + ".wav";
	
    /*QString systemcall = "lame --decode \""
            + std::string(mp3name) + "\" \"" + std::string(wavname) + "\" &> /dev/null";*/
	//std::cout << systemcall << std::endl;
	/*if (system(systemcall.c_str())) {
		std::cerr << "genchords: importMP3: lame decoding of mp3 failed" <<std::endl;
...
void Sounddata::import(const QString soundfilename) {
    SF_INFO info;
    SNDFILE * sndFile = sf_open(soundfilename.toUtf8().constData(), SFM_READ, &info);
	if (!sndFile) {
		std::cerr << "import: soundfile error: " << sf_strerror(sndFile) << std::endl;
		throw "soundfileerror";
	}
